...
int main() {
    ...
    // Операторы <= и >=
    if (num1 <= num2) {
        std::cout << "Число 1 меньше или равно числу 2" << std::endl;
    } else {
        std::cout << "Число 1 больше числа 2" << std::endl;
    }

    if (num1 >= num2) {
        std::cout << "Число 1 больше или равно числу 2" << std::endl;
    } else {
        std::cout << "Число 1 меньше числа 2" << std::endl;
    }

    // Пример использования перемещающего конструктора
    QuaternaryNumber num_moved = std::move(num1);  // num_moved перемещает данные из num1
    std::cout << "Число, перемещенное из числа 1: ";
    num_moved.Print_numbers();  // Ожидаемый вывод: 123
    std::cout << "Число 1 после перемещения: ";
    num1.Print_numbers();  // Ожидаемый вывод: 0 (пустой вектор)

    // Пример использования перемещающего оператора присваивания
    QuaternaryNumber num4;
    num4 = std::move(num2);  // num4 перемещает данные из num2
    std::cout << "Число 4 после перемещения числа 2: ";
    num4.Print_numbers();  // Ожидаемый вывод: 321
    std::cout << "Число 2 после перемещения: ";
    num2.Print_numbers();  // Ожидаемый вывод: 0 (пустой вектор)
